...
public class SkillItemController : MonoBehaviour
{
    ...
    /// <summary>
    /// 이 Row_Panel에서 표시해야 할 스킬 데이터 목록(최대 5개)
    /// </summary>
    /// <param name="rowData">한 줄에 표시할 스킬들 (최대 5개)</param>
    /// <param name="equipmentImagePrefab">단일 이미지 Prefab</param>
    public void SetData(List<SkillData> rowData, GameObject equipmentImagePrefab)
    {
        ...
        for (int i = 0; i < rowData.Count && i < equipmentImageParents.Length; i++)
        {
            ...
            {
                imgController.SkillSetData(skillData);
            }
        }
    }
}
